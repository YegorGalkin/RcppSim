...
#
summary_prev = 'D:/Rstuff/mathbio/MathBioSim/14_10_2020_sims_3d/summary.csv'
summary_prev = read_csv(summary_prev)
  
# Prepare directory for results
result_dir = './14_10_2020_sims_3d_iter2/'
...
params_all <- summary_prev%>%
  ...
  mutate(seed=1234L)%>%
  mutate(initial_population = desired_population, population_limit = 2e5L)%>%
  mutate(area_length_x=(desired_population/N)^(1/n_dim),periodic=TRUE,cell_count_x=10L)%>%
  mutate(n_samples = 50L)
