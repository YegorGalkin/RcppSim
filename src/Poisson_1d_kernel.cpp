...
struct Grid_1d_kernel
{
  ...
  double birth_inverse_cdf_step;
  int birth_inverse_cdf_nodes;
  boost::math::cubic_b_spline<double> birth_inverse_cdf_spline;
  ...
  void spawn_random()
  {
    ...
    double x_coord_new = parent_cell.coords_x[event_index] +
                         birth_inverse_cdf_spline(boost::random::uniform_01<>()(rng)) * (boost::random::bernoulli_distribution<>(0.5)(rng) * 2 - 1);
  ...
  double get_birth_inverse_cdf_spline_value(double at)
  {
    return birth_inverse_cdf_spline(at);
  }

  Grid_1d_kernel(Rcpp::List params) : time(), event_count(),
                                      cells(), cell_death_rates(), cell_population(),
                                      death_kernel_spline(), birth_inverse_cdf_spline(),
                                      total_population(), realtime_limit_reached(false)
  {
    ...
    birth_inverse_cdf_nodes = birth_inverse_cdf_y.size();
    birth_inverse_cdf_step = 1.0 / (birth_inverse_cdf_nodes - 1);
    ...
    birth_inverse_cdf_spline = cubic_b_spline<double>(birth_inverse_cdf_y.begin(), birth_inverse_cdf_y.end(), 0, birth_inverse_cdf_step);
...
RCPP_MODULE(poisson_1d_kernel_module)
{
  ...
  class_<Grid_1d_kernel>("poisson_1d_kernel")
      ...
      .field_readonly("birth_inverse_cdf_nodes", &Grid_1d_kernel::birth_inverse_cdf_nodes)
      .field_readonly("birth_inverse_cdf_step", &Grid_1d_kernel::birth_inverse_cdf_step)
      ...
      .method("birth_inverse_cdf_spline_at", &Grid_1d_kernel::get_birth_inverse_cdf_spline_value)
      ...
      .field_readonly("realtime_limit", &Grid_1d_kernel::realtime_limit)
      .field_readonly("realtime_limit_reached", &Grid_1d_kernel::realtime_limit_reached);
}
#endif
